...
func (c Config) toDTClientOptionsV2() []dtclient.OptionV2 {
	...
	if c.Proxy != "" {
		options = append(options, dtclient.WithV2BaseOptions(dtclient.WithProxy(c.Proxy, c.NoProxy)))
	}

	if c.SkipCertCheck {
		options = append(options, dtclient.WithV2BaseOptions(dtclient.WithSkipCertificateValidation(c.SkipCertCheck)))
	}
