...
func WithHPA(t *testing.T) features.Feature {
	...
	builder.Assess("check if the otelc statefulset has replicas set to 1", k8sstatefulset.WaitForSpecReplicas(testDynakube.OtelCollectorStatefulsetName(), testDynakube.Namespace, 1))
...
func updateReplicas(dk *dynakube.DynaKube, replicas *int32) features.Func {
	return func(ctx context.Context, t *testing.T, envConfig *envconf.Config) context.Context {
