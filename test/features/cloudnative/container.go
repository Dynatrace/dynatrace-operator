...
func AssessActiveGateContainer(builder *features.FeatureBuilder, dynakube *dynatracev1beta1.DynaKube, trustedCAs []byte) {
	builder.Assess("certificates are propagated to ActiveGate container", checkActiveGateContainer(dynakube, trustedCAs))
}

func checkActiveGateContainer(dynakube *dynatracev1beta1.DynaKube, trustedCAs []byte) features.Func {
	return func(ctx context.Context, t *testing.T, envConfig *envconf.Config) context.Context {
		resources := envConfig.Client().Resources()
