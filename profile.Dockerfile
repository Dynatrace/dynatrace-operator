...
RUN CGO_ENABLED=1 CGO_CFLAGS="-O2 -Wno-return-local-addr" go test -covermode=atomic -ldflags="${GO_LINKER_ARGS}" -coverpkg=github.com/open-cluster-management/endpoint-operator/pkg/... -c -tags testrunmain ./src/cmd -o ./build/_output/bin/dynatrace-operator
...
ENTRYPOINT ["/usr/local/bin/entrypoint_profiling"]
