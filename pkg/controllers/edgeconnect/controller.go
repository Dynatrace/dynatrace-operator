...
func getEdgeConnectByName(ctx context.Context, edgeConnectClient edgeconnectClient.Client, name string) (edgeconnectClient.GetResponse, error) {
	log := logd.FromContext(ctx)
	...
	if len(ecs.EdgeConnects) == 1 {
		log.Debug("Found one EdgeConnect objects with matching name", "count", ecs.EdgeConnects)

		return ecs.EdgeConnects[0], nil
	}

	log.Debug("No EdgeConnect object found with matching name", "count", ecs.EdgeConnects)
