...
type edgeConnectClientBuilderType func(ctx context.Context, ec *edgeconnect.EdgeConnect, oauthCredentials oauthCredentialsType, customCA []byte) (edgeconnectClient.APIClient, error)
...
func (controller *Controller) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if deletionTimestamp := ec.GetDeletionTimestamp(); deletionTimestamp != nil {
		log.Debug("EdgeConnect object shall be deleted", "timestamp", deletionTimestamp.String())

		return reconcile.Result{}, controller.reconcileEdgeConnectDeletion(ctx, ec)
	}

	log.Debug("EdgeConnect object needs reconcile")
...
func (controller *Controller) reconcileEdgeConnectDeletion(ctx context.Context, ec *edgeconnect.EdgeConnect) error {
	...
	log = log.WithValues("namespace", ec.Namespace, "name", ec.Name, "scenario", "deletion")

	log.Info("reconciling EdgeConnect deletion", "name", ec.Name, "namespace", ec.Namespace)
	...
	tenantEdgeConnect, err := getEdgeConnectByName(ctx, edgeConnectClient, ec.Name)
	if err != nil {
		log.Debug("failed to get EdgeConnect by name")
	...
	case tenantEdgeConnect.ID == "":
		log.Info("EdgeConnect not found on the tenant")
	...
	return edgeConnectClient.DeleteEdgeConnect(ctx, tenantEdgeConnect.ID)
}

func (controller *Controller) deleteConnectionSetting(ctx context.Context, edgeConnectClient edgeconnectClient.APIClient, ec *edgeconnect.EdgeConnect) error {
	envSetting, err := GetConnectionSetting(ctx, edgeConnectClient, ec.Name, ec.Namespace, ec.Status.KubeSystemUID)
	if err != nil {
		return err
	}
...
func (controller *Controller) reconcileEdgeConnect(ctx context.Context, ec *edgeconnect.EdgeConnect) (reconcile.Result, error) {
	log := logd.FromContext(ctx)
	log = log.WithValues("namespace", ec.Namespace, "name", ec.Name)
	...
	if err != nil {
		...
		log.Debug("error reconciling EdgeConnect, setting phase 'Error'")
	} else {
		log.Debug("moving EdgeConnect to correct phase")
		ec.Status.SetPhase(controller.determineEdgeConnectPhase(ctx, ec))
	}
	...
	log.Info("reconciling EdgeConnect done")

	if err != nil {
		log.Debug("reconcileEdgeConnect error")
...
func (controller *Controller) reconcileEdgeConnectCR(ctx context.Context, ec *edgeconnect.EdgeConnect) error {
	...
	if err := controller.updateFinalizers(ctx, ec); err != nil {
		log.Debug("updating finalizers failed")
	...
	if ec.Status.KubeSystemUID == "" {
		log.Debug("reconcile EdgeConnect kube-system UID")
	...
	if ec.IsProvisionerModeEnabled() {
		log.Debug("reconcile EdgeConnect provisioner")

		return controller.reconcileEdgeConnectProvisioner(ctx, ec)
	}

	log.Debug("reconcile regular EdgeConnect")
...
func (controller *Controller) updateEdgeConnectStatus(ctx context.Context, ec *edgeconnect.EdgeConnect) error {
	...
	if k8serrors.IsConflict(err) {
		log.Info("could not update EdgeConnect status due to conflict")
	...
	log.Info("EdgeConnect status updated", "timestamp", ec.Status.UpdatedTimestamp)
...
func (controller *Controller) reconcileEdgeConnectRegular(ctx context.Context, ec *edgeconnect.EdgeConnect) error {
	...
	desiredDeployment := deployment.New(ctx, ec)
	log = log.WithValues("namespace", ec.Namespace, "name", ec.Name, "deploymentName", desiredDeployment.Name)
	...
	if err != nil {
		log.Info("could not create or update deployment for EdgeConnect")
...
func (controller *Controller) reconcileEdgeConnectProvisioner(ctx context.Context, ec *edgeconnect.EdgeConnect) error { //nolint:revive
	...
	edgeConnectClient, err := controller.buildEdgeConnectClient(ctx, ec)
	if err != nil {
		log.Debug("unable to build EdgeConnect client")
	...
	if tenantEdgeConnect.ID != "" {
		if edgeConnectIDFromSecret == "" {
			...
			if err := edgeConnectClient.DeleteEdgeConnect(ctx, tenantEdgeConnect.ID); err != nil {
				return err
			}
...
func (controller *Controller) buildEdgeConnectClient(ctx context.Context, ec *edgeconnect.EdgeConnect) (edgeconnectClient.APIClient, error) {
...
func getEdgeConnectByName(ctx context.Context, edgeConnectClient edgeconnectClient.APIClient, name string) (edgeconnectClient.APIResponse, error) {
	...
	if len(ecs) > 1 {
		log.Debug("Found multiple EdgeConnect objects with the same name", "count", ecs)

		return edgeconnectClient.APIResponse{}, errors.New("many EdgeConnects have the same name")
	}

	if len(ecs) == 1 {
		log.Debug("Found one EdgeConnect objects with matching name", "count", ecs)

		return ecs[0], nil
	}

	log.Debug("No EdgeConnect object found with matching name", "count", ecs)
...
func (controller *Controller) getEdgeConnectIDFromClientSecret(ctx context.Context, ec *edgeconnect.EdgeConnect) (string, error) {
	...
	log = log.WithValues("namespace", ec.Namespace, "name", ec.Name, "clientSecretName", clientSecretName)
	...
	if err != nil {
		if k8serrors.IsNotFound(errors.Cause(err)) {
			log.Debug("EdgeConnect client secret not found")

			return "", nil
		} else {
			log.Debug("EdgeConnect client secret query failed")
...
func (controller *Controller) createEdgeConnect(ctx context.Context, edgeConnectClient edgeconnectClient.APIClient, ec *edgeconnect.EdgeConnect) error {
	...
	if err != nil {
		log.Debug("creating EdgeConnect failed")
	...
	if err != nil {
		log.Debug("unable to create EdgeConnect secret")
	...
	if err != nil {
		log.Debug("could not create or update secret for edge-connect client")

		return errors.WithStack(err)
	}

	log.Debug("EdgeConnect created")

	return nil
}

func (controller *Controller) updateEdgeConnect(ctx context.Context, edgeConnectClient edgeconnectClient.APIClient, ec *edgeconnect.EdgeConnect) error {
	log := logd.FromContext(ctx)
	log = log.WithValues("namespace", ec.Namespace, "name", ec.Name)

	secret, err := controller.secrets.Get(ctx, types.NamespacedName{Name: ec.ClientSecretName(), Namespace: ec.Namespace})
	if err != nil {
		log.Debug("EdgeConnect ID token not found")
	...
	oauthClientID, err := k8ssecret.ExtractToken(secret, consts.KeyEdgeConnectOauthClientID)
	if err != nil {
		log.Debug("EdgeConnect OAuth client token not found")

		return err
	}

	edgeConnectResponse, err := edgeConnectClient.GetEdgeConnect(ctx, id)
	if err != nil {
		log.Debug("EdgeConnect object not found")

		return errors.WithStack(err)
	}

	if slices.Equal(ec.HostPatterns(), edgeConnectResponse.HostPatterns) {
		log.Debug("EdgeConnect host patterns in response match", "patterns", ec.Spec.HostPatterns)
	...
	if err != nil {
		log.Debug("updating EdgeConnect failed")

		return errors.WithStack(err)
	}

	log.Debug("EdgeConnect updated")
...
func (controller *Controller) createOrUpdateEdgeConnectDeploymentAndSettings(ctx context.Context, ec *edgeconnect.EdgeConnect) error {
	...
	desiredDeployment.Spec.Template.Annotations[consts.EdgeConnectAnnotationSecretHash] = secretHash
	log = log.WithValues("deploymentName", desiredDeployment.Name)

	if err := controllerutil.SetControllerReference(ec, desiredDeployment, scheme.Scheme); err != nil {
		log.Debug("Could not set controller reference")
	...
	_, err = k8sdeployment.Query(controller.client, controller.apiReader).WithOwner(ec).CreateOrUpdate(ctx, desiredDeployment)
	if err != nil {
		log.Debug("could not create or update deployment for EdgeConnect")
	...
	if ec.IsK8SAutomationEnabled() {
		...
		err = controller.createOrUpdateConnectionSetting(ctx, edgeConnectClient, ec, edgeConnectToken)
		if err != nil {
			log.Debug("creating EdgeConnect connection setting failed")

			return err
		}

		log.Debug("EdgeConnect deployment created/updated successfully")
	}

	return nil
}

func (controller *Controller) createOrUpdateConnectionSetting(ctx context.Context, edgeConnectClient edgeconnectClient.APIClient, ec *edgeconnect.EdgeConnect, latestToken string) error {
	log := logd.FromContext(ctx)
	log = log.WithValues("namespace", ec.Namespace, "name", ec.Name)

	envSetting, err := GetConnectionSetting(ctx, edgeConnectClient, ec.Name, ec.Namespace, ec.Status.KubeSystemUID)
	if err != nil {
		log.Info("Failed getting EdgeConnect connection setting object")

		return err
	}

	if (envSetting == edgeconnectClient.EnvironmentSetting{}) {
		log.Debug("Creating edgeconnectClient connection setting object...")
	...
	if (envSetting != edgeconnectClient.EnvironmentSetting{}) {
		if envSetting.Value.Token != latestToken {
			log.Debug("Updating EdgeConnect connection setting object...")
...
func (controller *Controller) createOrUpdateEdgeConnectConfigSecret(ctx context.Context, ec *edgeconnect.EdgeConnect) (token string, hash string, err error) {
	...
	if err != nil {
		if k8serrors.IsNotFound(err) || errors.Is(err, ErrTokenNotFound) || errors.Is(err, ErrUnsupportedConfigFileVersion) {
			log.Debug("creating new token", "error", err.Error())
