...
func (checker *CorrectnessChecker) migrateHostMounts(ctx context.Context) {
	...
	for _, dk := range dks {
		...
		if err != nil {
			log.Error(err, "malformed APIURL for dynakube, skipping host dir migration for it", "apiUrl", dk.APIURL())
		...
		if err != nil {
			...
			log.Error(err, "failed to check deprecated host dir existence, skipping host dir migration for it", "apiUrl", dk.APIURL())
