...
func New(fs afero.Afero, apiReader client.Reader, path metadata.PathResolver) *Cleaner {
	return &Cleaner{
		fs:        fs,
		apiReader: apiReader,
		path:      path,
...
func (c Cleaner) run(ctx context.Context) error {
	...
	if err != nil {
		log.Info("failed to list available dynakubes, skipping unused binaries cleanup")
	...
	err = c.removeUnusedBinaries(dks, fsState)
	if err != nil {
		return err
	}
