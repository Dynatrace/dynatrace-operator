...
func TestRemoveOldSharedBinaries(t *testing.T) {
	t.Run("empty fs -> error", func(t *testing.T) {
		cleaner := createCleaner(t)

		keptBins := map[string]bool{}

		err := cleaner.removeOldSharedBinaries(keptBins)

		require.Error(t, err)
	})
	t.Run("empty shared dir -> no error", func(t *testing.T) {
		cleaner := createCleaner(t)
...
func createAppMonDk(t *testing.T, name, apiUrl string) dynakube.DynaKube {
	...
	dk.Spec.OneAgent.ApplicationMonitoring = &dynakube.ApplicationMonitoringSpec{
		UseCSIDriver: true, // will be redundant after merge to main
	}
