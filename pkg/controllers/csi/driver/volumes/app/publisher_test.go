...
func TestHasTooManyMountAttempts(t *testing.T) {
	t.Run(`initial try`, func(t *testing.T) {
		...
		bindCfg := &csivolumes.BindConfig{
			TenantUUID:       testTenantUUID,
			MaxMountAttempts: dynakube.DefaultMaxFailedCsiMountAttempts,
		}
...
func mockFailedPublishedVolume(t *testing.T, publisher *AppVolumePublisher) {
	mockUrlDynakubeMetadata(t, publisher)
	err := publisher.db.InsertVolume(context.Background(), metadata.NewVolume(testVolumeId, testPodUID, testAgentVersion, testTenantUUID, dynakube.DefaultMaxFailedCsiMountAttempts+1))
	require.NoError(t, err)
}
...
func mockImageDynakubeMetadata(t *testing.T, publisher *AppVolumePublisher) {
	err := publisher.db.InsertDynakube(context.Background(), metadata.NewDynakube(testDynakubeName, testTenantUUID, "", testImageDigest, dynakube.DefaultMaxFailedCsiMountAttempts))
	require.NoError(t, err)
}
