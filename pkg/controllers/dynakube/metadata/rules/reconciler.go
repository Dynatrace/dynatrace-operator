...
func (r *Reconciler) Reconcile(ctx context.Context) error {
	...
	if !conditions.IsOptionalScopeAvailable(r.dk, dtclient.ConditionTypeAPITokenSettingsRead) {
		...
		conditions.SetStatusOptionalScopeMissing(r.dk.Conditions(), conditionType, "Metadata-enrichment rules are not set in the status because the settings.read optional scope is not available")
