...
func TestReconciler_Reconcile(t *testing.T) {
	...
	t.Run("ALL sub reconciler runs if AG is enabled", func(t *testing.T) {
		...
		r := &Reconciler{
			...
			istioReconciler:      mockDynaKubeReconciler(t),
	...
	t.Run("Create AG capability (creation and deletion)", func(t *testing.T) {
		...
		r.istioReconciler = mockDynaKubeReconciler(t)
...
func TestExtensionControllerRequiresActiveGate(t *testing.T) {
	t.Run("no activegate is created when extensions are disabled in dk, and no capability is configured", func(t *testing.T) {
		...
		r.istioReconciler = newMockDynakubeReconciler(t)
...
func TestServiceCreation(t *testing.T) {
	...
	t.Run("service exposes all ports for every capabilities", func(t *testing.T) {
		...
		for capName, expectedPorts := range expectedCapabilityPorts {
			...
			reconciler := NewReconciler(fakeClient, fakeClient, dk, dynatraceClient, nil).(*Reconciler)
			...
			reconciler.istioReconciler = mockDynaKubeReconciler(t)
...
func mockDynaKubeReconciler(t *testing.T) dynakubeReconciler {
	t.Helper()

	reconciler := newMockDynakubeReconciler(t)
	reconciler.EXPECT().Reconcile(anyCtx, anyDynakube).Return(nil)
