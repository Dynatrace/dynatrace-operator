...
func createActivegateReconcilerBuilder(reconciler controllers.Reconciler) activegate.ReconcilerBuilder {
	return func(_ client.Client, _ client.Reader, _ *dynakubev1beta3.DynaKube, _ dtclient.Client, _ *istio.Client, _ token.Tokens) controllers.Reconciler {
		return reconciler
	}
}
...
func TestGetDynakube(t *testing.T) {
	t.Run("get dynakube", func(t *testing.T) {
		...
		assert.Equal(t, testName, dk.Name)
		assert.Equal(t, testNamespace, dk.Namespace)
		assert.NotNil(t, dk.Spec.OneAgent.CloudNativeFullStack)
	})
...
func createTenantSecrets(dynakube *dynakube.DynaKube) []client.Object {
