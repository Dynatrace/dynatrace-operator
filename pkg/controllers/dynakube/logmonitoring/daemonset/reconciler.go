...
func (r *Reconciler) Reconcile(ctx context.Context, imageClient dtimage.Client, dk *dynakube.DynaKube) error {
	...
	imageURI, err := registry.ResolveImage(ctx, imageClient, dk.FF().IsPublicRegistry(), dk.PublicRegistryOverride(), dtimage.LogModule, &dk.Spec.Templates.LogMonitoring.ImageRef)
	if err != nil {
		return err
	}
