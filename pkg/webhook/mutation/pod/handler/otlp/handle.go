...
func (h *Handler) Handle(mutationRequest *dtwebhook.MutationRequest) error {
	...
	if h.envVarMutator.IsEnabled(mutationRequest.BaseRequest) {
		if !h.isTokenSecretPresent(
			...
			log.Info("required input secret not present, skipping OTLP injection", "podName", mutationRequest.PodName(), "namespace", mutationRequest.Namespace.Name)
		...
		if !h.isExporterActiveGateCertSecretPresent(mutationRequest,
			...
			log.Info("required ActiveGate cert secret not present, skipping OTLP injection", "podName", mutationRequest.PodName(), "namespace", mutationRequest.Namespace.Name)
