...
const (
	...
	// General.
	AnnotationFeaturePublicRegistry = AnnotationPrefix + "public-registry"
	AnnotationFeatureNoProxy        = AnnotationPrefix + "no-proxy"

	// CSI.
	AnnotationFeatureMaxFailedCsiMountAttempts = AnnotationPrefix + "max-csi-mount-attempts"
	AnnotationFeatureMaxCsiMountTimeout        = AnnotationPrefix + "max-csi-mount-timeout"
	AnnotationFeatureReadOnlyCsiVolume         = AnnotationPrefix + "injection-readonly-volume"
...
func (f *FeatureFlags) GetNoProxy() string {
	return f.getRawValue(AnnotationFeatureNoProxy)
}

func (f *FeatureFlags) GetMaxFailedCsiMountAttempts() int {
	maxCsiMountAttemptsValue := f.getIntValue(AnnotationFeatureMaxFailedCsiMountAttempts, DefaultMaxFailedCsiMountAttempts)
...
func (f *FeatureFlags) GetMaxCSIRetryTimeout() time.Duration {
	maxCsiMountTimeoutValue := f.getRawValue(AnnotationFeatureMaxCsiMountTimeout)
...
func (f *FeatureFlags) ISCsiVolumeReadOnly() bool {
	return f.getRawValue(AnnotationFeatureReadOnlyCsiVolume) == truePhrase
}

func (f *FeatureFlags) IsPublicRegistryEnabled() bool {
	return f.getRawValue(AnnotationFeaturePublicRegistry) == truePhrase
}
