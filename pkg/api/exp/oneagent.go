...
func (f *FeatureFlags) GetIgnoredNamespaces() []string {
	raw := f.getRawValue(IgnoredNamespacesAnnotation)
	if raw == "" {
		return f.getDefaultIgnoredNamespaces()
	}
...
func (f *FeatureFlags) getDefaultIgnoredNamespaces() []string {
	defaultIgnoredNamespaces := []string{
		// fmt.Sprintf("^%s$", f.Namespace),
		"^kube-.*",
		"^openshift(-.*)?",
		"^gke-.*",
		"^gmp-.*",
...
// IsOneAgentProxyIgnored is a feature flag to ignore the proxy for oneAgents when set in CR.
...
func (f *FeatureFlags) GetOneAgentInitialConnectRetry() int {
	defaultValue := -1
	...
	// if ffValue == defaultValue && f.Spec.EnableIstio {
