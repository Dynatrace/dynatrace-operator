...
const (
	...
	warningOtelCollectorIgnoredTemplate = "The Dynakube's `spec.templates.otelCollector` section is skipped as the `spec.telemetryIngest` section is not configured."
)
...
func ignoredOtelCollectorTemplate(_ context.Context, _ *Validator, dk *dynakube.DynaKube) string {
	if !dk.TelemetryIngest().IsEnabled() && (dk.Spec.Templates.OpenTelemetryCollector.ImageRef.Repository != "" || dk.Spec.Templates.OpenTelemetryCollector.ImageRef.Tag != "") {
