...
func CheckVersion(ctx context.Context, apiReader client.Reader, crdName string) (error) {
	...
	if len(appVersion) == 0 {
		return errors.Errorf("%s env missing, can't check for CRD version", k8senv.AppVersion)
	}
	...
	if !ok {
		return errors.Errorf("missing version label '%s' on %s, mismatch found", k8slabel.AppVersionLabel, crdName)
	}
