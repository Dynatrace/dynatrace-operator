package zip

import (
	"encoding/base64"
	"io"
	"os"
	"path/filepath"
	"testing"

	"github.com/Dynatrace/dynatrace-operator/pkg/controllers/csi/metadata"
	"github.com/Dynatrace/dynatrace-operator/pkg/injection/codemodule/installer/common"
	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

const (
	testZipDirName  = "test"
	testZipFilename = "test.txt"

	TestRawZip  = ``
	TestRawGzip = ``

	// TestRawGzipWithSymlinks is a gzip archive containing:
	// - test.txt (regular file with content "you found the easter egg\n")
	// - testdir/ (directory)
	// - testdir/file2.txt (regular file with content "another test file\n")
	// - link_to_test.txt (symlink to test.txt)
	// - link_to_dir (symlink to testdir)
	TestRawGzipWithSymlinks = ``
)

func SetupInvalidTestZip(t *testing.T, tmpDir string) *os.File {
	t.Helper()

	tmpFile := filepath.Join(tmpDir, "tmp.zip")
	zipFile, err := os.OpenFile(tmpFile, os.O_CREATE|os.O_RDWR|os.O_TRUNC, os.ModePerm)
	require.NoError(t, err)

	faultyContent := []byte("DIE")
	_, err = zipFile.Write(faultyContent)
	require.NoError(t, err)

	return zipFile
}

func SetupTestArchive(t *testing.T, rawZip string) *os.File {
	t.Helper()

	tmpFile := filepath.Join(t.TempDir(), "test.zip")
	zipf, err := base64.StdEncoding.DecodeString(rawZip)
	require.NoError(t, err)

	zipFile, err := os.OpenFile(tmpFile, os.O_CREATE|os.O_RDWR|os.O_TRUNC, os.ModePerm)
	require.NoError(t, err)

	_, err = zipFile.Write(zipf)
	require.NoError(t, err)

	err = zipFile.Sync()
	require.NoError(t, err)

	_, err = zipFile.Seek(0, io.SeekStart)
	require.NoError(t, err)

	return zipFile
}

func createTestExtractor() Extractor {
	return NewOneAgentExtractor(metadata.PathResolver{})
}

func testUnpackedArchive(t *testing.T, tmpDir string) {
	t.Helper()

	assert.FileExists(t, filepath.Join(tmpDir, testZipFilename))
	assert.FileExists(t, filepath.Join(tmpDir, testZipDirName, testZipFilename))
	assert.FileExists(t, filepath.Join(tmpDir, testZipDirName, testZipDirName, testZipFilename))
	assert.FileExists(t, filepath.Join(tmpDir, common.AgentConfDirPath, common.RuxitConfFileName))

	info, err := os.Stat(filepath.Join(tmpDir, testZipFilename))
	require.NoError(t, err)
	require.NotNil(t, info)
	assert.False(t, info.IsDir())
	assert.Equal(t, int64(25), info.Size())

	info, err = os.Stat(filepath.Join(tmpDir, testZipDirName, testZipFilename))
	require.NoError(t, err)
	require.NotNil(t, info)
	assert.False(t, info.IsDir())
	assert.Equal(t, int64(25), info.Size())

	info, err = os.Stat(filepath.Join(tmpDir, testZipDirName, testZipDirName, testZipFilename))
	require.NoError(t, err)
	require.NotNil(t, info)
	assert.False(t, info.IsDir())
	assert.Equal(t, int64(25), info.Size())

	info, err = os.Stat(filepath.Join(tmpDir, common.AgentConfDirPath, common.RuxitConfFileName))
	require.NoError(t, err)
	require.NotNil(t, info)
	assert.False(t, info.IsDir())
	assert.Equal(t, int64(25), info.Size())
}
