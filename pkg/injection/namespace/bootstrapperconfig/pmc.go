...
func (s *SecretGenerator) getCachedPMC(ctx context.Context, dk *dynakube.DynaKube) (*oneagent.ProcessModuleConfig, error) {
	var pmConfig *oneagent.ProcessModuleConfig
	...
	if !k8sconditions.IsOutdated(s.timeProvider, dk, ConfigConditionType) {
		...
		if err != nil && !k8serrors.IsNotFound(err) {
			...
			pmConfig, err = oneagent.NewProcessModuleConfig(source.Data[pmc.InputFileName])
			if err != nil {
