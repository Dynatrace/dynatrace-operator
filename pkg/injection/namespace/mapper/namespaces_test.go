...
func TestMatchForNamespaceNothingEverything(t *testing.T) {
	...
	dynakubes := []*dynatracev1beta2.DynaKube{
		createDynakubeWithAppInject("appMonitoring-1", nil, nil),
		createDynakubeWithAppInject("appMonitoring-2", matchLabels, nil),
	}
...
func TestMapFromNamespace(t *testing.T) {
	...
	dk := createDynakubeWithMultipleFeatures("appMonitoring-1", labels)
	...
	t.Run("Error, 2 dynakubes point to same namespace", func(t *testing.T) {
		dk2 := createDynakubeWithAppInject("appMonitoring-2", labels, nil)
