...
func TestNewCertificatesViaFs(t *testing.T) {
	...
	certificates, err := newCertificatesViaFs(fs)
...
func prepTestFs(t *testing.T) afero.Fs {
	...
	require.NoError(t, fs.MkdirAll(consts.TrustedCAsAgentInitDirMount, 0770))

	file, err = fs.OpenFile(filepath.Join(consts.TrustedCAsAgentInitDirMount, "custom.pem"), os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0770)
	require.NoError(t, err)
	require.NotNil(t, file)
