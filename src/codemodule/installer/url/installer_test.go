...
import (
	"fmt"
	zip2 "github.com/Dynatrace/dynatrace-operator/src/codemodule/installer/zip"
	"io"
	"os"
	"testing"
	...
	t.Run(`error unzipping file`, func(t *testing.T) {
		...
		dtc.
			...
			Run(func(args mock.Arguments) {
				...
				zipFile := zip2.SetupInvalidTestZip(t, fs)
		...
		installer := &Installer{
			fs:        fs,
			dtc:       dtc,
			extractor: zip2.NewOneAgentExtractor(fs, metadata.PathResolver{}),
			props: &Properties{
				Os:     dtclient.OsUnix,
				Type:   dtclient.InstallerTypePaaS,
				Flavor: arch.FlavorMultidistro,
	...
	t.Run(`downloading and unzipping agent via version`, func(t *testing.T) {
		...
		dtc.
			...
			Run(func(args mock.Arguments) {
				...
				zipFile := zip2.SetupTestArchive(t, fs, zip2.TestRawZip)
				defer func() { _ = zipFile.Close() }()
