...
func ValidateCertificateExpiration(certData []byte, renewalThreshold time.Duration, log logr.Logger) (bool, error) {
	if block, _ := pem.Decode(certData); block == nil {
		...
		return false, err
	} else if time.Now().After(cert.NotAfter.Add(-renewalThreshold)) {
