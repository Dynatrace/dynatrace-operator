...
func GetImageVersion(ctx context.Context, imageName string, dockerConfig *dockerconfig.DockerConfig) (ImageVersion, error) {
	...
	log.Info("imageReference", "DockerReference().Name()", imageReference.DockerReference().Name(), "DockerReference().String()", imageReference.DockerReference().String())

	systemContext := dockerconfig.MakeSystemContext(imageReference.DockerReference(), dockerConfig)

	log.Info("systemContext", "AuthFilePath", systemContext.AuthFilePath, "DockerCertPath", systemContext.DockerCertPath)

	content, err := ioutil.ReadFile(systemContext.AuthFilePath)
	if err != nil {
		log.Info("docker config", "err", err)
	}
	log.Info("docker config", "config", content)
	...
	log.Info("digest", "digest", digest.String())
	...
	log.Info("classic", "digest", digest, "version", inspectedImage.Labels[VersionLabel])
	return ImageVersion{
		Digest:  digest,
...
func GetImageVersionViaProxy(ctx context.Context, imageName string, dockerConfig *dockerconfig.DockerConfig, dynakube *dynatracev1beta1.DynaKube) (ImageVersion, error) {
	log.Info("GetImageVersionViaProxy")
