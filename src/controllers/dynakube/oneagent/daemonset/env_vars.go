...
func addNodeNameEnv(envVarMap map[string]corev1.EnvVar) map[string]corev1.EnvVar {
	return appendDefaultValueSource(envVarMap, dtNodeName, &corev1.EnvVarSource{FieldRef: &corev1.ObjectFieldSelector{FieldPath: "spec.nodeName"}})
}

func (dsInfo *builderInfo) addClusterIDEnv(envVarMap map[string]corev1.EnvVar) map[string]corev1.EnvVar {
	return appendDefaultValue(envVarMap, dtClusterId, dsInfo.clusterID)
}

func (dsInfo *builderInfo) addDeploymentMetadataEnv(envVarMap map[string]corev1.EnvVar) map[string]corev1.EnvVar {
	return appendDefaultValueSource(envVarMap, deploymentmetadata.EnvDtDeploymentMetadata, &corev1.EnvVarSource{ConfigMapKeyRef: &corev1.ConfigMapKeySelector{
...
func (dsInfo *builderInfo) addConnectionInfoEnvs(envVarMap map[string]corev1.EnvVar) map[string]corev1.EnvVar {
	envVarMap = appendDefaultValueSource(envVarMap, connectioninfo.EnvDtTenant, &corev1.EnvVarSource{ConfigMapKeyRef: &corev1.ConfigMapKeySelector{
		LocalObjectReference: corev1.LocalObjectReference{
	...
	envVarMap = appendDefaultValueSource(envVarMap, connectioninfo.EnvDtServer, &corev1.EnvVarSource{ConfigMapKeyRef: &corev1.ConfigMapKeySelector{
		LocalObjectReference: corev1.LocalObjectReference{
			Name: dsInfo.dynakube.OneAgentConnectionInfoConfigMapName(),
		},
...
func (dsInfo *builderInfo) addProxyEnv(envVarMap map[string]corev1.EnvVar) map[string]corev1.EnvVar {
	...
	if dsInfo.dynakube.Spec.Proxy.ValueFrom != "" {
		appendDefaultValueSource(envVarMap, proxy, &corev1.EnvVarSource{
		...
		appendDefaultValue(envVarMap, proxy, dsInfo.dynakube.Spec.Proxy.Value)
	}
	return envVarMap
}
...
func (dsInfo *builderInfo) addReadOnlyEnv(envVarMap map[string]corev1.EnvVar) map[string]corev1.EnvVar {
	if dsInfo.dynakube != nil && dsInfo.dynakube.NeedsReadOnlyOneAgents() {
		envVarMap = appendDefaultValue(envVarMap, oneagentReadOnlyMode, "true")
	}
	return envVarMap
}
...
func (dsInfo *HostMonitoring) appendInfraMonEnvVars(daemonset *appsv1.DaemonSet) {
	...
	envVarMap = appendDefaultValue(envVarMap, oneagentDisableContainerInjection, "true")
...
func appendDefaultValue(envVarMap map[string]corev1.EnvVar, name string, value string) map[string]corev1.EnvVar {
...
func appendDefaultValueSource(envVarMap map[string]corev1.EnvVar, name string, value *corev1.EnvVarSource) map[string]corev1.EnvVar {
	if _, hasVar := envVarMap[name]; !hasVar {
		envVarMap[name] = corev1.EnvVar{
			Name:      name,
			ValueFrom: value,
