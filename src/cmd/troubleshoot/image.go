...
func verifyAllImagesAvailable(ctx context.Context, baseLog logr.Logger, apiReader client.Reader, httpClient *http.Client, pullSecret v1.Secret, dynakube *dynatracev1beta1.DynaKube) error {
	...
	if dynakube.NeedsOneAgent() {
		verifyImageIsAvailable(ctx, log, keyChain, transport, dynakube, componentOneAgent, false)
		verifyImageIsAvailable(ctx, log, keyChain, transport, dynakube, componentCodeModules, true)
	}
	if dynakube.NeedsActiveGate() {
		verifyImageIsAvailable(ctx, log, keyChain, transport, dynakube, componentActiveGate, false)
	}
	return nil
}
func verifyImageIsAvailable(ctx context.Context, log logr.Logger, keyChain authn.Keychain, transport *http.Transport, dynakube *dynatracev1beta1.DynaKube, comp component, proxyWarning bool) {
	...
	err := tryImagePull(ctx, keyChain, transport, image)
	if err != nil {
...
func verifyImageIsAvailableOld(ctx context.Context, log logr.Logger, apiReader client.Reader, httpClient *http.Client, pullSecret v1.Secret, dynakube *dynatracev1beta1.DynaKube, comp component, proxyWarning bool) {
	image, isCustomImage := comp.getImage(dynakube)
	if comp.SkipImageCheck(image) {
		logErrorf(log, "Unknown %s image", comp.String())
		return
	...
	err := tryImagePullOld(ctx, apiReader, httpClient, pullSecret, dynakube, image)
	if err != nil {
		logErrorf(log, "Pulling %s image %s failed: %v", componentName, image, err)
	} else {
		logOkf(log, "%s image %s can be successfully pulled", componentName, image)
...
func tryImagePullOld(ctx context.Context, apiReader client.Reader, httpClient *http.Client, pullSecret v1.Secret, dynakube *dynatracev1beta1.DynaKube, image string) error {
	imageReference, err := name.ParseReference(image)
	if err != nil {
		return err
	}
