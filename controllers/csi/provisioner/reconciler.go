...
func (r *OneAgentProvisioner) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	latestRuxitProcResponse, storedRevision, err := r.getRuxitProcResponse(dtc, dynakube.TenantUUID)
	if err != nil {
	...
	if updatedVersion, err := installAgentCfg.updateAgent(dynakube.LatestVersion, dynakube.TenantUUID, storedRevision, latestRuxitProcResponse); err != nil {
	...
	err = r.writeRuxitCache(dynakube.TenantUUID, latestRuxitProcResponse)
	if err != nil {
		return reconcile.Result{}, err
	}
