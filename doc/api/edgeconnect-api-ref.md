...
|Parameter|Description|Default value|Data type|
|:-|:-|:-|:-|
|`digest`|Digest pins the image to a specific content-addressable hash in the OCI<br/>`<algorithm>:<hex>` form (e.g. `sha256:abc...`). When set, the tag is ignored<br/>when rendering the image reference.|-|string|
|`pullPolicy`|Image pull policy to use|-|string|
