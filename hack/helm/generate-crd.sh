...
# Add the common labels by finding the line 'name: prometheusmonitorings.dynatrace.com' and inserting labels before it
awk 'BEGIN{inserted=0} /name: prometheusmonitorings.dynatrace.com/ && !inserted {print "  labels:"; print "    {{- include \"dynatrace-operator.commonLabels\" . | nindent 4 }}"; inserted=1} {print}' "${SOURCE_CRD_FILE}" > "${SOURCE_CRD_DIR}/tmp_crd"
mv "${SOURCE_CRD_DIR}/tmp_crd" "${SOURCE_CRD_FILE}"
