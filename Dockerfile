...
RUN mkdir -p /tmp/rootfs-base /tmp/rootfs-dependency /tmp/rootfs-diff
COPY --from=base / /tmp/rootfs-base
...
RUN dnf install --installroot /tmp/rootfs-dependency \
 ...
 && dnf install rsync --setopt install_weak_deps=false --nodocs -y \
 && rsync -aic --dry-run /tmp/rootfs-dependency/ /tmp/rootfs-base/ | grep '^>f' | sed 's#^[^ ]* ##' | tee /tmp/changeset \
 && rsync --files-from=/tmp/changeset /tmp/rootfs-dependency/ /tmp/rootfs-diff/

FROM base

COPY --from=dependency /tmp/rootfs-diff /
